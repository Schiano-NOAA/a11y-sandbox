# Early testing of methods for creating taggable tables with various packages
...
# This compiles! An easy minimum reproducible example
```{latex}
\DocumentMetadata{
  lang        = en,
  pdfstandard = ua-2,
...
Pasting this text into the SAR_species_skeleton.tex file (the Tables section), then compiling it, works
```{latex}
\tagpdfsetup{table/header-rows={1,2}}
\begin{tabular}{lr}
\multicolumn{2}{c}{Example}\\
...
# Diving deeper:
```{latex}
% took kable's output and add the tagpdfsetup line first
% now, commenting out some other lines

% -this does tag it as a table. Had to change header=rows to 1, not 1&2
...
# Does it tag as a table when compiled? Yes, but it's tagging the header cells as data cells
```{r}
...
# The only option I can find that tags the header cells as such is to add this line before the other kable text:
```{latex}
\tagpdfsetup{table/header-rows={1}}
...
```{r}
# First, export the raw data used to create a landings table into a csv in the tagged-tbls branch of stockplotr
raw_tbl <- read.csv(
  fs::path(
    getwd(),
    "tables",
